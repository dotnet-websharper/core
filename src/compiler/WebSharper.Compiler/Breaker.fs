...
let rec removeLets expr =
    ...
    | Let (var, value, I.Function ([x], true, _, (I.ExprStatement(I.Application (I.Var f, [I.Var y], _)) | I.Return(I.Application (I.Var f, [I.Var y], _)))))
        when f = var && x = y ->
            value
...
let optimize expr =
    match expr with
    // eta-reduction
    | Function (vars, true, _, I.Return (I.Application (f, args, { KnownLength = Some i })))
        when List.length args = i && sameVars vars args && isPureExpr f && VarsNotUsed(vars).Get(f) ->
        f
    // created for sprintf
    | Application(CurriedLambda(i :: vars, ret, I.Application (Var j, [expr], _), true), [ I.Function([x], true, _, I.Return (Var y))], _) when i = j && x = y ->
        CurriedLambda(vars, ret, expr)
    // created for F# string interpolation
    | Application(Lets(bindings, I.Function ([i], true, _, I.Return (I.Application (Var j, [expr], _)))), [ I.Function([x], true, _, I.Return (Var y))], _) when i = j && x = y ->
