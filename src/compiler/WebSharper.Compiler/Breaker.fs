...
let rec removeLets expr =
    ...
    | Let (var, value, I.Function ([x], (I.ExprStatement(I.Application (I.Var f, [I.Var y], _)) | I.Return(I.Application (I.Var f, [I.Var y], _)))))
        when f = var && x = y ->
            value
...
let optimize expr =
    ...
    | Application(TupledLambda(vars, body, isReturn), [ INewArray args ], { KnownLength = Some _ })
        when vars.Length = args.Length && not (needsScoping vars args body) ->
        if isReturn then
            List.foldBack2 bind vars args body
        else 
            List.foldBack2 bind vars args (Sequential [body; Value Null])
    | Application(I.ItemGet(I.Function (vars, I.Return body), I.Value (String "apply"), _), [ I.Value Null; argArr ], _) ->
        List.foldBack2 bind vars (List.init vars.Length (fun i -> argArr.[Value (Int i)])) body                   
    | Application (I.Function (args, (I.ExprStatement body | I.Return body)), xs, { KnownLength = Some _ }) 
    ...
    | Application (I.Function (_, (I.Empty | I.Block [])), xs, _) ->
        Sequential xs
    | Sequential [a] ->
        a
...
and private breakSt statement : Statement seq =
    ...
    | Return a ->
        ...
        match brA.Body with
        | ResultExpr (I.Application (I.Function (args, body), xs, _))
